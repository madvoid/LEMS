...
const int ftm_powerL = 24;          	                // Lower 5TM sensor power pin
const int ftm_powerU = 26;          	                // Upper 5TM sensor power pin
const int tn9_data = 2;				        // TN9 data pin
const int tn9_clk = 3;    				// TN9 clock pin
const int tn9_action = 4;          			// TN9 action pin
const int li_pin = A8;		        		// Li200 Input pin
const int green_led = 9;		        	// Datalogger green LED
const int red_led = 8;				        // Datalogger red LED
const int hih_ss = 25;					// HIH4030 ADC Slave Select
const int li_ss = 53;                                // Licor ADC Slave Select
OneWire oneWire(23);					// Initialize OneWire Device on pin 23
...
void setup(){
  ...
  logfile.println("Millis,Datetime,Dallas Amb,Rel Hum,Temp Corrected Rel Hum,Pressure,BMP Amb,IR,TN9 Amb,Soil Lower Temp,Soil Lower Mois,Soil Upper Temp,Soil Upper Mois,Sunlight");
...
void loop(){
  ...
  if(tn9_irflag && tn9_ambflag){			        // If successful IR and Ambient reading...
    ...
    li_val = mcp3201(li_ss);					
    // PLACE BIT CONVERTING EQUATION HERE

    hih_raw = mcp3201(hih_ss);
    ...
    delay(9000);						// Wait for next reading
